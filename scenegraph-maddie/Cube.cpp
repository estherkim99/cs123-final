...
void Cube::createVertices(int p1, int p2) {
    ...
    POS_XY->setVertexData(&pos_xy[0], pos_xy.size(), VBO::GEOMETRY_LAYOUT::LAYOUT_TRIANGLE_STRIP, pos_xy.size()/6);
    ...
    NEG_XY->setVertexData(&neg_xy[0], neg_xy.size(), VBO::GEOMETRY_LAYOUT::LAYOUT_TRIANGLE_STRIP, neg_xy.size()/6);
    ...
    POS_ZY->setVertexData(&pos_zy[0], pos_zy.size(), VBO::GEOMETRY_LAYOUT::LAYOUT_TRIANGLE_STRIP, pos_zy.size()/6);
    ...
    NEG_ZY->setVertexData(&neg_zy[0], neg_zy.size(), VBO::GEOMETRY_LAYOUT::LAYOUT_TRIANGLE_STRIP, neg_zy.size()/6);
    ...
    POS_XZ->setVertexData(&pos_xz[0], pos_xz.size(), VBO::GEOMETRY_LAYOUT::LAYOUT_TRIANGLE_STRIP, pos_xz.size()/6);
    ...
    NEG_XZ->setVertexData(&neg_xz[0], neg_xz.size(), VBO::GEOMETRY_LAYOUT::LAYOUT_TRIANGLE_STRIP, neg_xz.size()/6);
...
std::vector<float> Cube::createFace(int p1, glm::vec3 normal, glm::vec3 topLeft, glm::vec3 topRight, glm::vec3 botLeft, glm::vec3 botRight) {
    ...
    for (int i = 0; i < p1; i++) {
        for (int j = 0; j < p1+1; j++) {
            vector.push_back(topLeft[0] + i * h_offset[0] + j * w_offset[0]);
            vector.push_back(topLeft[1] + i * h_offset[1] + j * w_offset[1]);
            vector.push_back(topLeft[2] + i * h_offset[2] + j * w_offset[2]);
            vector.insert(vector.end(), {normal[0], normal[1], normal[2]});

            vector.push_back(topLeft[0] + (i+1) * h_offset[0] + j * w_offset[0]);
            vector.push_back(topLeft[1] + (i+1) * h_offset[1] + j * w_offset[1]);
            vector.push_back(topLeft[2] + (i+1) * h_offset[2] + j * w_offset[2]);
            vector.insert(vector.end(), {normal[0], normal[1], normal[2]});
