...
void Cylinder::createVertices(int p1, int p2) {
    ...
    BARREL->setVertexData(&barrel[0], barrel.size(), VBO::GEOMETRY_LAYOUT::LAYOUT_TRIANGLE_STRIP, barrel.size()/8);
    ...
    BARREL->setAttribute(ShaderAttrib::TEXCOORD0, 2, 24, VBOAttribMarker::DATA_TYPE::FLOAT, false);
    BARREL->buildVAO();
}
...
std::vector<float> Cylinder::createBarrel(int p1, int p2, int multiplier) {
    ...
    for (int j = 0; j < p2; j++) { // one "side" of barrel per iter., drawn from top to bottom
        for (int k = 0; k < p1 + 1; k++) {
            float offset = 1.f / p1;
            glm::vec3 point = glm::vec3(points[j][0], points[j][1] - k * offset, points[j][2]);
            barrel.insert(barrel.end(), {point[0], point[1], point[2]});
            barrel.insert(barrel.end(), {2 * points[j][0], 0, 2 * points[j][2]}); // normal
            glm::vec2 uv = TextureUtils::cylinderToUV(glm::vec4(point.xyz(), 1));
            barrel.insert(barrel.end(), {uv[0], uv[1]});

            if (j != p2 - 1) { // if not on last side, go to next point
                point = glm::vec3(points[j+1][0], points[j+1][1] - k * offset, points[j+1][2]);
                barrel.insert(barrel.end(), {point[0], point[1], point[2]});
                barrel.insert(barrel.end(), {2 * points[j+1][0], 0, 2 * points[j+1][2]}); // normal
                glm::vec2 uv = TextureUtils::cylinderToUV(glm::vec4(point.xyz(), 1));
                barrel.insert(barrel.end(), {uv[0], uv[1]});
            } else { // else, go to first point
                point = glm::vec3(points[0][0], points[0][1] - k * offset, points[0][2]);
                barrel.insert(barrel.end(), {point[0], point[1], point[2]});
