    ...
    for (int rowcounter = rowstart; rowcounter < rowend; rowcounter++) {
        for (int colcounter = colstart; colcounter < colend; colcounter++) {
            ...
            if (dst < m_radius + 1) {
                ...
                RGBA pixel = pix[canvas_index];
                unsigned char newR = lerp(pixel.r, static_cast<unsigned char>(getRed()), m * getAlpha() / 256);
                unsigned char newG = lerp(pixel.g, static_cast<unsigned char>(getGreen()), m * getAlpha() / 256);
                unsigned char newB = lerp(pixel.b, static_cast<unsigned char>(getBlue()), m * getAlpha() / 256);
